...
export const api = {
  ...
  post<T = any>(
    ...
    return got
      .post(apiConfig.endpoint + endpoint, buildStandardHeaders(options))
      .json();
  },
};
...
export async function fetchAllGameData(
  ...
  const overview = (
    ...
    ).json<BaseApiResponse<EnrichedInfoOverview>>()
  ).data;
  const loadContexts$: Promise<any>[] = Object.entries(overview.context).map(
    async ([contextKey, contextInfo]) => {
      const contextKeyLowercase = contextKey.toLowerCase();
      if (
        contextKeyLowercase === 'cdnaddress' ||
        (only.length &&
          !only.find(
            (check) => check === contextKey || check === contextKeyLowercase,
          ))
      ) {
        return;
      }
      const data = (
        await got(contextInfo.link).json<BaseApiResponse<AllGameInfo[]>>()
      ).data;
