...
import { fingerprintAlgo, GameDataFile } from '@base/entity/GameDataFile';
...
createConnection()
  .then(async () => {
    ...
    for (const gameKey of availableGameKeys) {
      ...
      if (!game) {
        console.error(
          `[ERROR] Game ${gameKey} does not exist in the database.`,
        );
        continue;
      }
      ...
      for (const dataFolderName of dataFolders) {
        ...
        const infoAggregate: InfoAggregate = require(join(
          dataFolder,
          'Info.json',
        ));
