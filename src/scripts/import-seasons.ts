...
import { GameDataFile } from '../entity/GameDataFile';
...
createConnection().then(async connection => {
  ...
  for (const game of Object.values(games)) {
    ...
    for (const dalcomSeasonIdStr of Object.keys(byCode)) {
      ...
      const similarSeasonsQuery = await WorldRecordSeasons.createQueryBuilder('season')
        .where(
          'season.dateStart < :upper AND season.dateStart > :lower',
          { lower: dateStartLower.toDate(), upper: dateStartUpper.toDate() }
        )
      ...
      if (similarSeasons.length) {
        console.log(` [Similar] Game ${game.key} season #${dalcomSeasonId} has similar season, updating, not adding. (${dateStart})`);
