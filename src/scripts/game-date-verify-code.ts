...
async function main(gameKey: string, contextKey: string, verifyKey: string) {
  ...
  const contextDataVersions = (await api.get<BaseApiResponse<number[]>>(contextDataEndpoint)).data;
  ...
  const promises$ = contextDataVersions.map(async version => {
    const contextData = await api.get<AllGameInfo[]>(contextDataEndpoint + '/' + version + '?noWrap=true');
  ...
  // console.log(map);
  console.log(Object.values(map).every(byCode => Object.keys(byCode).length === 1));
  console.log('Done.');
}
