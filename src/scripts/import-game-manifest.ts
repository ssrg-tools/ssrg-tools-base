...
import { GameManifest } from '@base/entity/GameManifest';
import { GameManifest as GameManifestType } from '@base/definitions/data/gamemanifest';
...
createConnection()
  .then(async () => {
    ...
    if (existingManifests) {
      console.log(
        `${game.key}: Manifest for ${game.key}@${version} already exists.`,
      );
    } else {
    ...
    if (version === manifestContents.ActiveVersion_Android) {
      console.log(
        `${game.key}: Game app version ${game.key}@${version} already up-2-date.`,
      );
      return;
    }
    ...
    console.log(
      `${game.key}: Updated game app to ${manifestContents.ActiveVersion_Android}`,
    );
  })
  .then(() => process.exit(0))
  .catch((reason) => {
