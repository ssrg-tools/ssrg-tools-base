...
export class ArtistsImportArtists1600365081098 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const themesRaw: { album, cardCount, gameId }[] = await queryRunner.query(`SELECT themes.album, themes.cardCount, themes.game_id as gameId
    ...
    for await (const theme of themes) {
      const artistResult = await connection.createQueryBuilder(queryRunner)
        .insert()
        .into('artists')
        .values({ name: theme.album, cardCount: theme.cardCount, gameId: theme.gameId, guid: theme.guid })
        .execute();
      ...
      const membersResult = await connection.createQueryBuilder(queryRunner)
        .insert()
        .into('artists_members')
        .values(_.range(0, theme.cardCount).map(memberOffset => ({
