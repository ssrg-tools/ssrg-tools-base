...
export class SongBeatmapInfo1597072188176 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `songs` ADD UNIQUE INDEX `IDX_dcae9d25b142b942fff5fd4c83` (`dalcom_song_filename`)',
    );
    await queryRunner.query(
      "ALTER TABLE `songs` ADD `beatmap_fingerprint` varchar(255) NULL COMMENT 'game internal song filename' DEFAULT NULL",
    );
    await queryRunner.query(
      'ALTER TABLE `songs` ADD UNIQUE INDEX `IDX_a8d2f0f709399856f601de34c3` (`beatmap_fingerprint`)',
    );
    await queryRunner.query(
      "ALTER TABLE `songs` ADD `beatmap_date_processed` datetime NULL COMMENT 'date when the beatmaps had been processed' DEFAULT NULL AFTER `dalcom_song_filename`",
    );
    await queryRunner.query(
      'ALTER TABLE `song_beatmaps` ADD CONSTRAINT `FK_c8d12ea5178e0709deb03433d82` FOREIGN KEY (`song_id`) REFERENCES `songs`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT',
    );
  }
