...
export class AddGameAssetArchive1619569017057 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `files_gameasset_archive` ADD CONSTRAINT `FK_5539a59caa01d26f73a9f508554` FOREIGN KEY (`fileId`) REFERENCES `files`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT');
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('DROP INDEX `byFileId` ON `files_gameasset_archive`');
