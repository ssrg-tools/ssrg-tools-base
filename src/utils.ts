...
export function scoreBonusCalculate(
  dateAlbum: Date | Moment,
  dateArtists: (Date | Moment)[] = [],
  dateNow: Date | Moment = new Date(),
) {
  let bonus = 0;
...
function buildPrefixKeysFromQuery(query: object, prefix) {
  ...
  for (const extractKey in query) {
    if (
      Object.prototype.hasOwnProperty.call(query, extractKey) &&
      extractKey.length > prefix.length &&
      extractKey.startsWith(prefix)
    ) {
